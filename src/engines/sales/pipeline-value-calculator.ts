...
const engine: ToolEngine = {
  ...
  faq: [
    ...
    { q: "What is pipeline value?", a: "Pipeline value = sum of all open opportunities × deal value. For 10 deals at $50K each: $500K pipeline. Used for: 1) Forecasting, 2) Capacity planning, 3) Performance tracking. Track: total, weighted by stage, by source, by rep. Most: separate from committed revenue. Tools: Salesforce, HubSpot, Pipedrive. Most teams: track weighted pipeline for forecasting. Aim: 3-4x coverage." },
    { q: "What is weighted pipeline value?", a: "Weighted pipeline = pipeline value × close probability. Example: $500K pipeline × 30% close = $150K weighted. By stage: 1) Discovery 5%, 2) Demo 10%, 3) Evaluation 25%, 4) Negotiation 50%, 5) Commit 80%. Most: use stage-based weights. Sum: forecast. Most teams: 80% forecast accuracy with discipline. Tools: Salesforce, Clari. Update weekly. Most: weighted vs actual = variance to track." },
    { q: "How do I track pipeline value vs committed?", a: "Track 3 categories: 1) Commit (90%+ close): 100% weighted, 2) Best case (50-80% close): 50% weighted, 3) Pipeline (10-50% close): 25% weighted. Most: avoid binary (in/out). Tools: 1-2-3 framework. Most reps: 1 commit per month. Total: sum weighted by category. Avoid: \"everything is commit\" (over-forecasting). Discipline: 80% of commit = actual." },
    { q: "What is a good pipeline value for $1M quota?", a: "For $1M quota: 1) New business: $3-4M pipeline, 2) Expansion: $1-1.5M, 3) Total: $4-5M. By segment: 1) SMB (high volume): 5-6x, 2) Mid-market: 3-4x, 3) Enterprise (low volume): 5-10x. Most: 3-4x for healthy. By stage: distribute across stages (20% early, 50% mid, 30% late). Avoid: 80% in early stage (low quality)." },
    { q: "How do I avoid pipeline bloat?", a: "Pipeline bloat: too much low-quality pipeline. Symptoms: 1) Coverage >5x, 2) Low close rates (<15%), 3) Stale deals, 4) Many small deals. Tactics: 1) Better qualification (BANT, MEDDIC), 2) Stage discipline (no skipping), 3) Disqualify early, 4) Cap deal size, 5) Marketing qualified leads only. Most: cut pipeline 30-50% but increase close rate. Quality over quantity." },
    { q: "What is the difference between pipeline and forecast?", a: "Pipeline: all open deals. Forecast: expected revenue in period. Pipeline × close rate = forecast. Pipeline: $5M. Forecast: $1.5M (30% close rate). For board: forecast is what matters. For sales: pipeline is what you build. Most: track both. Pipeline for reps, forecast for managers. Tools: Salesforce, Clari, BoostUp. Discipline: 80% accuracy on forecast is good. Most teams: improve quarterly." },
    { q: "How do I use pipeline value for rep performance?", a: "Rep metrics: 1) Pipeline created (vs quota), 2) Pipeline coverage (3-4x), 3) Close rate (25-35%), 4) Average deal size, 5) Cycle time. Reps at quota: usually 3-4x coverage + 30% close. Below: low pipeline or low quality. Above: low quality (junk pipeline). Most: weekly 1:1s to review pipeline. Top tactic: pipeline build rate (rep adds 1-2x quota per quarter). Aim: 2-3x quota in pipeline per quarter." },
    { q: "What is the average sales pipeline value for SaaS?", a: "Benchmarks: 1) <$1M ARR: $2-5M pipeline, 2) $1-5M ARR: $5-20M, 3) $5-20M ARR: $20-80M, 4) $20-100M ARR: $80-300M. By stage: weighted pipeline = 25-30% of total pipeline. Most: 3-4x quota in pipeline. For $5M quota: $15-20M pipeline. Tools: Salesforce, HubSpot. Track weekly. Most: validate quarterly. Avoid: stale pipeline (skews metrics)." },
  ],
  howToUse: [
