...
const engine: ToolEngine = {
  ...
  faq: [
    ...
    { q: "What is pipeline coverage ratio?", a: "Pipeline coverage = total pipeline value / quota. Examples: $5M quota, $10M pipeline = 2x coverage. Benchmarks: 1) <2x: under-pipelined, 2) 2-3x: minimum, 3) 3-4x: healthy, 4) 4x+: bloated. Most: 3x is the standard. Why: 50% close rate on 2x = 100% of quota. Realistic: 25-30% close on 3x = 75-90% of quota. Plan: 3-4x for healthy forecast." },
    { q: "What is the right coverage ratio for SaaS sales?", a: "Benchmarks: 1) New business: 3-4x, 2) Expansion/upsell: 2-3x, 3) Renewals: 1.2-1.5x. By stage: 1) Early (PMF): 5-10x (high uncertainty), 2) Growth: 3-4x, 3) Scale: 2-3x (predictable). Most: 3-4x for new business. Lower for renewals (higher close rate). By segment: SMB 3-4x, mid-market 3-4x, enterprise 5-10x (long cycles). Quarterly: validate vs quota." },
    { q: "How do I improve pipeline coverage?", a: "Tactics: 1) Outbound (cold email, calls), 2) Inbound (more content, ads), 3) Referral program, 4) Partnerships, 5) Events, 6) PLG (product-led growth). Most: 30-50% coverage lift in 6 months. Top tactics: 1) Outbound + 2) Inbound. For low coverage: 1) More prospecting, 2) Better qualification, 3) Marketing lead gen. Most teams: 50% of pipeline from outbound, 30% inbound, 20% other." },
    { q: "What is the difference between pipeline coverage and pipeline value?", a: "Pipeline value: total $ in pipeline. Pipeline coverage: pipeline value / quota. Pipeline value: absolute. Coverage: relative to goal. For forecasting: coverage is what matters. For tracking: both. Tools: Salesforce, HubSpot. Most: track coverage weekly. <3x: need to push prospecting. >5x: bloated or unrealistic. Forecast: weighted by close probability. Most: 3x is healthy. Plan: never below 3x." },
    { q: "How do I handle stale pipeline?", a: "Stale: pipeline >90 days old. Tactics: 1) Stage-gate (no movement = disqualified), 2) Quarterly pipeline reviews, 3) Auto-disqualify after 6 months, 4) Re-engage or remove. Most: 30-50% of pipeline is stale. Clean regularly. Most teams: weekly reviews. Stale pipeline: not counted in coverage. Trim: every 90 days. Re-engagement: emails + calls. Avoid: zombie pipeline (skews forecast)." },
    { q: "What is the relationship between coverage and close rate?", a: "Coverage × close rate = quota. For 3x coverage × 30% close = 90% of quota. For 3x × 50% close = 150% of quota. For 2x × 50% = 100%. For 2x × 30% = 60%. Most: target 25-35% close rate on weighted pipeline. If close rate <20%: pipeline quality issue. If close rate >40%: pipeline is too conservative. Most: forecast by stage (deals in late stage close higher)." },
    { q: "Should pipeline include expansion deals?", a: "Depends. Most: separate expansion pipeline from new business. Reasons: 1) Different close rate (higher for expansion), 2) Different sales motion, 3) Different rep (often CSM). Coverage for expansion: 2-3x is fine (high close rate). Coverage for new business: 3-4x. Track: separate. For forecasting: combine weighted. Most: 30-50% of revenue from expansion in mature SaaS. Aim: 110% NRR means healthy expansion." },
    { q: "How do I forecast accurately with pipeline coverage?", a: "Forecast = (pipeline_value × close_rate) by stage. Stage-based close rates: 1) Discovery: 5-10%, 2) Demo: 10-20%, 3) Evaluation: 25-40%, 4) Negotiation: 50-70%, 5) Commit: 80%+. Most: 5-stage forecast. Tools: Salesforce (stage-based), Clari (AI). Best: history-based (actual close by stage). Most teams: 80%+ forecast accuracy with discipline. Track: actual vs forecast quarterly. Calibrate models." },
  ],
  howToUse: [
