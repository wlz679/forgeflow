...
const engine: ToolEngine = {
  ...
  faq: [
    ...
    { q: "What is search effectiveness?", a: "Search effectiveness = % of searches that lead to a resolution (article click + no follow-up ticket). Benchmarks: 60-80% is good. <50% = poor search or content. Track: searches, clicks, helpful %, ticket creation after click. Top performers: 85%+ effective. AI search (vector embeddings) lifts effectiveness 20-40% vs keyword search." },
    { q: "How do I improve KB search effectiveness?", a: "Tactics: 1) AI-powered search (vector embeddings, semantic), 2) Synonym handling (user says \"delete\" article says \"remove\"), 3) Auto-correct typos, 4) Better tagging, 5) Personalized results (by user role/plan). Most teams: 20-40% improvement with AI search. Investment: $5K-50K for AI search vendor. ROI: 3-10x via reduced tickets." },
    { q: "What is the difference between search and findability?", a: "Search: user types query, gets results. Findability: user finds answer without searching (good IA, navigation, related articles). Findability is often cheaper to improve. Tactics: better IA, popular articles on landing page, \"you might also need\", clear categories. Avoid: only improving search when IA is the real problem." },
    { q: "How do I measure search quality?", a: "Track: 1) Searches with no results (target <5%), 2) Searches with click (target 50%+), 3) Click leading to ticket (target <20%), 4) Search refinements (target <2 per session). Tools: Algolia, Coveo, Searchspring, Elasticsearch. Use AI for natural language. Track weekly. Search log analysis: top missed queries drive content gaps." },
    { q: "What is the cost of poor search?", a: "Poor search: 1) Higher ticket volume (40% more), 2) Lower CSAT (users frustrated), 3) Higher support cost ($10-25 per ticket), 4) SEO traffic loss (Google indexes poor search pages). For a 10K-ticket/month support org: 30% effective search saves $30K-75K/year. Investment in AI search: $5K-50K. ROI: 3-15x." },
    { q: "Should I use Algolia, Elasticsearch, or AI search?", a: "Algolia: fast, good for simple use, $1-5K/month. Elasticsearch: customizable, open-source, requires ops. AWS Kendra: enterprise, smart, $5-20K/month. AI search (Coveo, Glean, Guru): best for complex queries, $10-50K/month. Pick based on: budget, technical resources, query complexity. Most B2B: AI search wins for >5K articles." },
    { q: "What is zero-result search handling?", a: "Zero-result searches = users searching for terms with no matching article. Track: top 20 zero-result queries. These are your content gaps. Action: 1) Write articles for top 10, 2) Improve search synonyms, 3) Suggest related articles. Top 20 zero-result queries likely drive 50% of unreached content. Reduce zero-result rate from 15% to <5% in 6 months." },
    { q: "How does search differ for mobile vs desktop?", a: "Mobile: 60%+ of support queries. Constraints: shorter queries, voice input, time pressure. Tactics: 1) Voice-first design, 2) Predictive search, 3) Larger tap targets, 4) Recent searches, 5) Quick links. Mobile search effectiveness is typically 10-20% lower than desktop. Optimize for mobile explicitly. Mobile-first KB is mandatory now." },
  ],
  howToUse: [
