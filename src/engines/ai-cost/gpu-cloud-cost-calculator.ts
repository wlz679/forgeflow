...
function calculate(inputs: Record<string, string>): string[] {
  ...
  out.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  out.push('• 🧭 Decision Question: 单纯看 GPU hourly rate 是陷阱，**核心问题是"实际 workload cost / 月 + idle time 浪费率 + vendor lock-in 风险"**。最便宜 ≠ 最划算。');
  out.push('• 🧭 Recommendation: (1) **rate < 中位数 30%** → 警惕 hidden cost（data egress / API gateway fee / 抢占 spot 不可靠）；(2) **中位数 ±20%** → 安全选型；(3) **> 中位数 30%** → 仅在 SLA / 合规硬性要求时付溢价；(4) **多 vendor 分散** → 降低 lock-in 但增加 ops 复杂度。');
  out.push('• 🧭 Key Uncertainty: (1) 不同 vendor pricing model 不一样（per-second vs per-hour vs reserved）；(2) GPU 类型差异大（A100 vs H100 vs L40S 不可比）；(3) data egress fee 可能吞掉 savings；(4) spot preempt 频率影响 production workload。');
  out.push('• 🧭 Next Action: (a) 跑 [OpenAI Token Calculator] 看 API vs 自托管 cost 对比；(b) 跑 [AI Training Cost Estimator] 看训练全周期 cost；(c) 跑 [Claude API Cost Calculator] 看主力模型成本曲线；(d) 决策前做 30 天 proof-of-concept 验证 vendor SLA。');
...
const customFn =
  ...
  "o.push('\\n\\n\\uD83E\\uDDED Decision Recommendation');" +
  "o.push('\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501\\u2501');" +
  "o.push('\\u2022 \\uD83E\\uDDED Decision Question: 单纯看 GPU hourly rate 是陷阱，**核心问题是\"实际 workload cost / 月 + idle time 浪费率 + vendor lock-in 风险\"**。最便宜 ≠ 最划算。');" +
  "o.push('\\u2022 \\uD83E\\uDDED Recommendation: (1) **rate < 中位数 30%** \\u2192 警惕 hidden cost（data egress / API gateway fee / 抢占 spot 不可靠）；(2) **中位数 ±20%** \\u2192 安全选型；(3) **> 中位数 30%** \\u2192 仅在 SLA / 合规硬性要求时付溢价；(4) **多 vendor 分散** \\u2192 降低 lock-in 但增加 ops 复杂度。');" +
  "o.push('\\u2022 \\uD83E\\uDDED Key Uncertainty: (1) 不同 vendor pricing model 不一样（per-second vs per-hour vs reserved）；(2) GPU 类型差异大（A100 vs H100 vs L40S 不可比）；(3) data egress fee 可能吞掉 savings；(4) spot preempt 频率影响 production workload。');" +
  "o.push('\\u2022 \\uD83E\\uDDED Next Action: (a) 跑 [OpenAI Token Calculator] 看 API vs 自托管 cost 对比；(b) 跑 [AI Training Cost Estimator] 看训练全周期 cost；(c) 跑 [Claude API Cost Calculator] 看主力模型成本曲线；(d) 决策前做 30 天 proof-of-concept 验证 vendor SLA。');" +
  "return o;";
...
const engine: ToolEngine = {
  ...
  presets: [
    ...
    { key: 'dev-box',    emoji: '🛠️', fields: { provider: 'runpod',     gpuType: 'A100',    gpuCount: '1', hoursPerDay: '8',  pricingTier: 'on-demand', includeStorage: 'yes' } },
    ...
    { key: 'cheapest-h200',       emoji: '🔬', fields: { provider: 'vastai',     gpuType: 'H200',    gpuCount: '1', hoursPerDay: '4',  pricingTier: 'spot',      includeStorage: 'no'  } },
  ...
  faq: [
    ...
    { q: "Which GPU is most cost-effective for AI workloads?", a: "For inference (production): A10G ($0.50-1/hour) or L4 ($0.50-1/hour) — best perf/watt. For training (small models): A100 40GB ($1-1.5/hour) — sweet spot. For training (large models): H100 80GB ($3-5/hour) — fastest. For serving: T4 ($0.30-0.50/hour) — cheapest for low-throughput workloads." },
    { q: "How do I compare AWS vs GCP vs Azure GPU pricing?", a: "AWS p4d.24xlarge (8x A100): $32/hour. GCP a2-ultragpu-8g (8x A100): $30/hour. Azure NDv5 (8x A100): $35/hour. CoreWeave, Lambda, RunPod (specialized): 30-50% cheaper. Spot/preemptible: 60-70% off but can be killed. Use spot for training, on-demand for serving." },
    { q: "What is reserved vs spot vs on-demand pricing?", a: "On-demand: pay per hour, fully available. Reserved: 30-60% off, 1-3 year commit. Spot: 60-80% off, can be killed anytime. Use spot for: training (can checkpoint resume), batch jobs. Use reserved for: predictable production workloads. Use on-demand for: dev environments, burst capacity." },
    { q: "How do I estimate GPU cost for inference serving?", a: "Throughput-based: tokens/sec × hours × GPU_count. For 100K tokens/hour at 5K tokens/sec/GPU on A10G: 20 GPUs × $0.75/hour × 24 × 30 = $10,800/month. Use autoscaling (scale to zero when idle). Use multi-GPU for high-throughput, single-GPU for low-latency." },
    { q: "What is the difference between H100 and A100?", a: "H100: 3x faster training, 4x faster inference vs A100, transformer engine, FP8 support. A100: 80GB HBM2e, mature ecosystem, 1.5-3 PFLOPs. For new training jobs: H100 is faster. For serving: A100 is cheaper except for compute-bound workloads. H100 is the best, A100 is the workhorse." },
    { q: "How do I optimize GPU utilization?", a: "Use batching (process 8-32 requests per batch). Use vLLM, TGI, TensorRT for high-throughput inference. Use dynamic batching for variable load. Use smaller models when possible (7B vs 70B = 10x throughput). Use quantization (INT8 = 2x throughput, 4x less memory). Monitor GPU utilization — aim for 70%+." },
    { q: "What are hidden costs in GPU cloud pricing?", a: "Storage ($0.10-0.20/GB/month). Egress ($0.05-0.12/GB after first free tier). Idle time (paid even if unused). Backup snapshots. Data transfer between regions. IP addresses. Load balancers. Always estimate total cost including storage + egress, not just hourly GPU rate." },
    { q: "How do I choose between self-hosting and APIs?", a: "Self-host: <$2K/month compute — prefer APIs. $2K-50K/month — gray zone, evaluate. >$50K/month — self-host often wins. Add: eng time for setup, monitoring, scaling. APIs: $0 cost upfront, elastic scaling, no infra. For prototype-to-low-scale, APIs win. For high-scale or data privacy, self-host." },
  ],
  howToUse: [
