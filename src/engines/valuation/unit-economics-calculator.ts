...
function calculateUnitEconomics(inputs: Record<string, string>): string[] {
  ...
  if (netContribution > 0) {
    ...
    if (cac > 0 && isFinite(paybackMonths)) {
      out.push("• Break-even customers/mo needed to amortize CAC:");
...
const engine: ToolEngine = {
  ...
  faq: [
    ...
    { q: "What are unit economics?", a: "Unit economics = revenue and costs per unit (per customer, per transaction). For SaaS: per customer = LTV, CAC, gross margin. Components: 1) ARPU, 2) Gross margin, 3) CAC, 4) LTV, 5) Payback, 6) NRR. Most: track per cohort. Healthy: 3:1 LTV:CAC, <12 month payback, 80%+ gross margin, 100%+ NRR. Investors: unit economics is the #1 thing they look at. Most: improve before scaling." },
    { q: "What is a good LTV:CAC ratio for SaaS?", a: "Benchmarks: 1) 1:1 = break-even, 2) 3:1 = healthy, 3) 5:1 = under-investing, 4) 10:1 = excellent. By stage: 1) Seed: 2:1+ (acceptable), 2) Series A: 3:1+, 3) Series B: 4:1+. For investors: 3:1 is the gate. Most: target 3-4:1. Top: 5:1+. Calculate: (LTV × gross_margin) / CAC. Be honest: use net LTV (after margin). Most: 3:1+ is healthy. Below 2:1 = scaling issues." },
    { q: "What is CAC payback and what is good?", a: "CAC payback = months to recover CAC from gross profit. For $1K CAC, $100/mo ARPU, 80% margin: payback = 1000/(100 × 0.8) = 12.5 months. Benchmarks: 1) <6 months: world-class, 2) 6-12 months: healthy, 3) 12-18 months: acceptable, 4) 18-24 months: concerning, 5) >24 months: critical. Most SaaS: 12-18 months. Top: <12. Best: <6. For each month reduction: 5-10% valuation lift." },
    { q: "What is gross margin and what is good for SaaS?", a: "Gross margin = (revenue - COGS) / revenue. COGS = hosting + support + payment fees. For $100 revenue, $20 COGS: 80% margin. Benchmarks: 1) 80%+: world-class SaaS, 2) 70-80%: good, 3) 60-70%: average, 4) <60%: services-heavy. Most SaaS: 75-85%. Top: 85%+. AWS, Stripe, Snowflake: 70-80%. Pure SaaS (no services): 80%+. Most: aim for 80%+. Lower margin: lower multiple." },
    { q: "How do I improve unit economics?", a: "Top tactics: 1) Reduce CAC (better targeting, SEO, referrals), 2) Increase LTV (lower churn, higher ARPU), 3) Increase gross margin (cheaper hosting, automation), 4) Faster payback (annual prepay). Most: 20-50% unit economics improvement in 12 months. Top tactics: 1) Reduce churn + 2) SEO. Most: 1 pp churn reduction = 10-20% LTV lift. Aim: 3:1 LTV:CAC + 12-month payback. Be patient: organic takes 6-12 months." },
    { q: "What is the relationship between unit economics and valuation?", a: "Better unit economics = higher multiple. For 3:1 LTV:CAC, 12-month payback, 80% margin: 8-12x ARR. For 5:1 LTV:CAC, 6-month payback, 85% margin: 15-20x. Top: 5:1+ LTV:CAC, <12 month payback, 85%+ margin = 20-30x. Investors: 3-4x for healthy. Most: aim for 3:1 minimum. Track: by cohort. Most teams: 5-10 pp unit economics improvement per year = 2-3x multiple lift." },
    { q: "What is the difference between gross margin and contribution margin?", a: "Gross margin: revenue - COGS. COGS = cost of delivering product. Contribution margin: gross margin - variable operating costs (some S&M, customer success). For SaaS: gross margin 80%, contribution margin 60-70%. For valuation: use gross margin. For unit economics: use contribution margin. Most: gross margin is the standard. Contribution margin: more conservative. Use: gross for headline, contribution for unit economics." },
    { q: "How do I model unit economics in financial projections?", a: "Model: 1) ARPU per cohort, 2) Gross margin %, 3) CAC, 4) Churn, 5) Compute LTV, payback, NRR. Per cohort. For 5-year model: 1) Year 1: low volume, high CAC, 2) Year 2-3: scaling, 3) Year 4-5: mature. Most: cohort-based (each cohort has its own retention). Tools: Excel, Fathom, Mosaic. Most: validate against actuals quarterly. Most teams: model by segment (SMB, mid-market, enterprise)." },
  ],
  howToUse: [
