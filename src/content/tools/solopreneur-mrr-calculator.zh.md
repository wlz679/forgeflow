...
预测来刻画真实增长。使用量定价（usage-based，如 Twilio API、AWS 按调用
计费）下，本月 MRR 不能直接乘客户数得出，需按 usage attribution 把当月账单
切到对应客户；seat-based（按席位，如 Figma、Notion）才是 MRR 框架原生
场景。月中升级会被拆成 prorated 金额，与合同 MRR 不一致；credit memo /
服务积分退款需从当月毛 MRR 扣减，否则高估 Net MRR 留存表现。
