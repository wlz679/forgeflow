...
这些基准来自：Mixpanel 的群组分析文档（行业留存曲线和衰减模型）、Amplitude 的群组留存方法学指南，以及 Recurly 订阅指标基准报告 2024（按垂直领域的群组留存曲线）。
...
- 群组留存按特定获客群组中在第 N 个月仍活跃的用户比例衡量 — 不控制群组规模或季节性。
- 衰减模型假设固定的留存曲线形状（线性或指数）；年付合约等阶跃流失场景应使用曲线拟合配合留存热图。
- 时间窗口默认 12 个月；消费品等短周期产品用 90 天或 30 天群组窗口。

## 常见误区

- 报告群组平均留存而非曲线留存 — 平均值掩盖真实形状（如早期陡降后平台）；永远报告曲线，不要报告单点数字。
- 混合主动流失和被动流失 — 主动（取消）是产品/营销信号；被动（支付失败）是账单信号；分开报告。
- 忽略群组规模 — 小群组（n<50）留存曲线噪声大；报告置信区间或在仪表盘隐藏小群组数据。
