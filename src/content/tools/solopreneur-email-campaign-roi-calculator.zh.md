...
这些基准来自：DMA（数据与营销协会）邮件营销委员会年度报告（行业 ROI 每花 1 美元回报 36-42 美元）、HubSpot 营销现状报告的邮件基准、Klaviyo 年度电商邮件基准报告，以及 Litmus 邮件营销 ROI 行业分析。
...
- 邮件 ROI 计算包含活动成本（平台费、设计、名单管理），不含订阅者获取成本；全漏斗 ROI 请配合 CAC 计算器。
- 营收归因假设邮件是最后接触转化驱动；首次或多点归因会产生不同数字。
- 打开率假设不含 Apple Mail 隐私保护预取（约 50% 打开）；点击率和转化率更可靠。

## 常见误区

- 把打开率作为主要成功指标 — Apple MPP 把打开率虚增 2-3 倍；优先看点击打开率和转化率。
- 把退订视为失败 — 单次发送 0.1-0.5% 退订率是健康水平；高活跃名单可以承受更频繁的发送。
- 全名单发送相同内容 — 细分 CTA（购物车放弃者、最近购买者、沉默用户）比广播邮件 ROI 高 3-5 倍。
