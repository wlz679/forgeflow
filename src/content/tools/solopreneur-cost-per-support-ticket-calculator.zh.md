...
另外要注意小型 SaaS 与企业级客户的支持成本方差极大：SMB 工单因平均
处理时长短（5–10 分钟）、自助分流比例高通常落在 $5–$15，而企业工单
因升级链长、定制环境配置、CSM 介入常常突破 $100/张——直接套用一个
加权均值会把企业线成本严重低估。隐藏成本也常被忽略：升级链 L1→L2→L3
会把单工单成本乘 3–5 倍，而一次糟糕支持体验造成的客户流失损失，往往
相当于数十张工单的挽回成本，应纳入下一层级的流失风险评估。
