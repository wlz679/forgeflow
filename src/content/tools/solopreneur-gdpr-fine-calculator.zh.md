...
上限是天花板，不是预测——实际罚款受违规严重程度、主观意图、缓解措施与配合度影响（IAPP 2024 强制力地图：中位实际罚款为上限的 0.5-1%，Meta 等极端案例达全球收入的 1-2%）。模型以欧盟为锚定；CCPA（按次 $2,500-$7,500，无收入上限）、LGPD、PIPL 敞口画像不同，需分别建模。行业乘数是基于历史案件均值的规划启发式，不是案件特定预测——监管机构依第 83(2) 条保留完全自由裁量权。
