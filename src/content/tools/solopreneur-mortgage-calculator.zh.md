...
本计算器把房价、首付、贷款年限和按揭利率，换算成你每月需还给银行的本息（principal-and-interest, P&I）金额，以及整个贷款周期付的总利息。它同时输出摊销里程碑（每月还款里本金 vs 利息的比例变化）和 LTV 比率——后者决定你是否会触发私人按揭保险（PMI）。看房前用来定预算；出价后用来对比 15 年 vs 30 年的总成本。摊销曲线前期主要在付利息：30 年贷款的第 1 年里，本金只占月供的 12–15%，到第 20 年才升到约 60%；这就是 15 年 vs 30 年总利息常差 $200,000 以上的根本原因。模型只覆盖 Fannie Mae / Freddie Mac 监管的合规贷款（conforming loan）——2026 年基准上限 $832,750，高房价县提升至 $1,249,125，超出此额度的 Jumbo 贷款由机构单独定价。
...
本计算器**只算本金和利息**。不含房产税、房屋保险、HOA 费、洪水险——这些加起来视地区和房价，每月可能再加 $300–$1,500；T&I（taxes & insurance）通常进入 escrow account，由银行代收代付，进一步拉高实际月供。模型也默认**固定利率贷款**；若你选的是 ARM（5/1、7/1 等可调利率），优惠期结束后月供会重置——典型 lifetime cap 为首期利率 +5%、annual cap 2%、首调上限 ±5%，adjustment interval 一般 6–12 个月，可显著放大月供波动。PMI 也只显示"触发 / 不触发"，不计入实际保费：按惯例首付 < 20% 强制 PMI，年化 0.5–1.5%，需等 LTV 降到 78% 以下才能取消。ARM、FHA 贷款（带 upfront MIP）、VA 贷款（带 funding fee）或非标准大额贷款，请直接用贷款机构的报价。
