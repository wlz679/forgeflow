...
工单分流分两步：用户在工单提交前的搜索框命中知识库文章（前端拦截），
或 AI 在工单创建后推荐相关文章（事后兜底）。前者贡献 Zendesk 2024 报告
15–30% 的 SaaS 自助分流率，远高于后者——只有两类命中都纳入统计，才能
反映真实拦截能力，避免高估知识库的实际减负价值。
...
知识库还有半衰期问题：API 改版、产品更名、计费规则调整后，旧文章误
差率会在半年内快速上升；行业惯例是核心文章每季度复核、长尾文章每半
年一次。另外搜索有效性波动大，搜索命中率（即用户在前 3 条结果里找
到答案的比例）才是真正可衡量的有效性指标，单纯看文章数量会严重误
导决策。
