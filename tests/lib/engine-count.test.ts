...
test(`engine registry contains exactly ${EXPECTED_ENGINE_COUNT} engines (drift detection)`, async () => {
  ...
  assert.equal(
    ...
    `Engine registry has ${getAllEngines().length} engines but EXPECTED_ENGINE_COUNT=${EXPECTED_ENGINE_COUNT}. Update tests/lib/engine-count.ts when adding/removing engines.`
  );
});
