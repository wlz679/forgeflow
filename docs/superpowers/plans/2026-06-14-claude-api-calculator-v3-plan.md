# Claude API Cost Calculator v3 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Upgrade Claude API Cost Calculator from 3-model basic calculator to v3 with 7 models, Prompt Caching, batch pricing, growth projections, cross-provider comparison, and CSS beautification — matching OpenAI Token Calculator v3 standard.

**Architecture:** Complete rewrite of `src/engines/claude-api-cost-calculator.ts` following the same pattern as `openai-token-calculator.ts`. All logic exists in both TypeScript `calculate()` (SSR) and a minified JS `customFn` (client-side). Output is plain text with `█`/`░` bar chart characters for SEO; client-side JS parses and renders CSS-styled HTML via `beautifyClaude()`.

**Tech Stack:** TypeScript, Astro SSR + client JS, shared `ToolEngine` interface
...
|------|--------|----------------|
| `src/engines/claude-api-cost-calculator.ts` | **Rewrite** | MODELS, calculate(), customFn, engine metadata |
| `src/data/tools.ts` | **Modify** | Sync inputs array |
| `src/i18n/translations.ts` | **Modify** | Add ~45 Claude-specific i18n keys |
| `src/pages/[lang]/[slug].astro` | **Modify** | Claude presets, token estimator, advanced collapse, beautifyClaude() |

---

### Task 1: MODELS constant + family metadata + helpers

**Files:**
- Modify: `src/engines/claude-api-cost-calculator.ts` (replace entire file)

- [ ] **Step 1: Write the new MODELS constant and all helper constants**

Replace the entire file content. Start with imports, then ModelInfo interface, MODELS, family constants, cache constants, presets, and helper functions:
...
// Token estimator (Claude tokenizer ≈1.3-1.5 tokens/word English, ~0.7 chars/token CJK)
...
```bash
git add src/engines/claude-api-cost-calculator.ts
git commit -m "feat(claude): MODELS constant — 7 models × 3 families with Prompt Caching metadata"
```

---

### Task 2: calculate() function — 7-section output

**Files:**
- Modify: `src/engines/claude-api-cost-calculator.ts` (append calculate() after helper functions)

- [ ] **Step 1: Write the calculate() function**
...
function calculate(inputs: Record<string, string>): string[] {
  ...
  if (cachingActive) {
    ...
    headerLine += ' | \u{1F4BE} Cache: ' + cacheReadHitRate + '% hit (' + ttlLabel + ')';
  }
  out.push(headerLine);
  out.push('');
  out.push(
  ...
  if (cachingActive) {
    out.push(
      '\u{1F4BE} Cache Write: ' + lc(cacheWriteTokens) + ' tokens/req (' +
  ...
  for (const c of allCosts) {
    ...
    const barChar = c.key === cheapest.key ? '░' : '█'; // ░ for cheapest, █ for others
  ...
  for (const c of selectedCosts) {
    ...
    if (cachingActive) {
      ...
      out.push('  \u{1F4BE} With caching: ' + fmt(c.monthlyCost) + ' (' + pctSaved + '% ' + sign + ')');
    }
    out.push('');
  }
  ...
  // ================================================================
  if (cachingActive) {
    const ref = cheapest; // Use cheapest model for breakdown example
    const ip = ref.inputPrice;
    const ttlLabel = cacheTTL === '1hour' ? '1-hour TTL' : '5-min TTL';
    const wmLabel = writeMult === 1.25 ? '1.25×' : '2×';
    out.push('\u{1F4BE} Prompt Caching Breakdown (' + ttlLabel + ' × ' + wmLabel + ' write)');
    out.push(SEP.repeat(54));
  ...
  out.push('\u{1F4A1} Premium buys: 1M context, best-in-class safety, superior code generation');
  out.push('');
...
Expected: No errors related to claude-api-cost-calculator.ts.

- [ ] **Step 3: Commit**

```bash
git add src/engines/claude-api-cost-calculator.ts
git commit -m "feat(claude): calculate() — 7-section output with Prompt Caching breakdown"
```

---

### Task 3: customFn — exact sync with calculate()

**Files:**
- Modify: `src/engines/claude-api-cost-calculator.ts` (replace customFn string after calculate())

- [ ] **Step 1: Write the customFn string**

The customFn must produce IDENTICAL output to calculate(). Append after calculate():

```typescript
const customFn =
  // --- MODELS map (full keys) ---
  "var M={" +
...
- [ ] **Step 2: Verify customFn produces valid output**

Run via a quick node test:

```bash
cd "D:\E\独立站\youtube-tools" && node -e "
const customFn = require('fs').readFileSync('src/engines/claude-api-cost-calculator.ts','utf8').match(/customFn\s*=\s*\"([\s\S]*?)\";/);
// If match found, evaluate it
const fn = new Function('inputs', (customFn[1]));
const result = fn({inputTokens:'1000',outputTokens:'500',requestsPerDay:'100'});
console.log(result.slice(0,5).join('\\n'));
"
```

Expected: First few lines of output matching calculate() format.

- [ ] **Step 3: Commit**

```bash
git add src/engines/claude-api-cost-calculator.ts
git commit -m "feat(claude): customFn — exact sync with 7-section calculate()"
```

---

### Task 4: Engine metadata — inputs, FAQ, howToUse, staticExamples

**Files:**
- Modify: `src/engines/claude-api-cost-calculator.ts` (append engine const after customFn)

- [ ] **Step 1: Write engine metadata with staticExamples generated from actual output**

First, run a script to generate the actual calculate() output, then append the engine block:
...
const engine: ToolEngine = {
  ...
  staticExamples: [
    // Generated from: calculate({inputTokens:'1000',outputTokens:'500',requestsPerDay:'100'})
  ...
  faq: [
    ...
    },
    {
      ...
      a: 'Claude uses a different tokenizer from OpenAI. For English text, Claude produces roughly 1.3-1.5 tokens/word (vs OpenAI\'s ~1.3). Opus 4.7 introduced a new tokenizer than can produce up to 35% more tokens for the same text compared to Opus 4.6. Always compare token counts when switching models.',
    },
    {
...
- [ ] **Step 2: Generate accurate staticExamples**

Run the actual calculate() function with default inputs to get real output:

```bash
cd "D:\E\独立站\youtube-tools" && node -e "
// Import the module and call calculate with defaults
const { calculate } = require('./src/engines/claude-api-cost-calculator.ts');
// If module import fails, manually call the function after build
"
```

Since ES module import may not work in plain node, instead verify by building and checking the SSR output:

```bash
cd "D:\E\独立站\youtube-tools" && pnpm build
```

Then manually copy the output from the built HTML's static example div. Replace the staticExamples array with the actual output.

- [ ] **Step 3: Commit**

```bash
git add src/engines/claude-api-cost-calculator.ts
git commit -m "feat(claude): engine metadata — 10 inputs, 7 FAQ, 6 howToUse, staticExamples"
```

---

### Task 5: Sync tools.ts

**Files:**
- Modify: `src/data/tools.ts` (find Claude entry and update inputs)

- [ ] **Step 1: Update Claude's inputs in tools.ts**

Find the Claude calculator entry in `src/data/tools.ts` and replace its `inputs` array with the 10 fields matching the engine definition. The tools.ts entry should look like:

```typescript
{
  slug: 'solopreneur-claude-api-cost-calculator',
  // ... existing fields (title, description, category, icon)
  inputs: [
    { name: 'models', label: 'Models', placeholder: 'claude-fable-5,claude-opus-4-8,claude-sonnet-4-6,claude-haiku-4-5', type: 'text' },
    { name: 'inputTokens', label: 'Input Tokens per Request', placeholder: 'e.g. 1000', type: 'number' },
    { name: 'outputTokens', label: 'Output Tokens per Request', placeholder: 'e.g. 500', type: 'number' },
...
git commit -m "feat(claude): sync tools.ts — 10 inputs matching engine"
```
...
- [ ] **Step 1: Add Claude-specific i18n keys**

Add translations under the `tools['solopreneur-claude-api-cost-calculator']` key. Include:

- 10 input labels + placeholders (en + zh)
- 7 FAQ Q&A pairs (en + zh)
- 6 howToUse steps (en + zh)
- 6 preset labels (en + zh)
- 7 model names (en is English name, zh is same)
- Section header labels: "Prompt Caching Breakdown", "Savings Insights", "Growth Projection", "Usage Scenarios", "Cost Comparison"
...
// ... etc for all 10 inputs

'tools.solopreneur-claude-api-cost-calculator.preset.small-project': { en: 'Small Project', zh: '小项目' },
'tools.solopreneur-claude-api-cost-calculator.preset.mid-scale': { en: 'Mid-Scale', zh: '中规模' },
'tools.solopreneur-claude-api-cost-calculator.preset.high-volume': { en: 'High Volume', zh: '大流量' },
'tools.solopreneur-claude-api-cost-calculator.preset.batch': { en: 'Batch Processing', zh: '批量处理' },
'tools.solopreneur-claude-api-cost-calculator.preset.heavy-cache': { en: 'Heavy Caching', zh: '重度缓存' },
'tools.solopreneur-claude-api-cost-calculator.preset.enterprise': { en: 'Enterprise', zh: '企业级' },

// FAQ: 7 pairs, each with .q and .a
// howToUse: 6 steps

// Model names
'tools.solopreneur-claude-api-cost-calculator.model.claude-fable-5': { en: 'Claude Fable 5', zh: 'Claude Fable 5' },
'tools.solopreneur-claude-api-cost-calculator.model.claude-opus-4-8': { en: 'Claude Opus 4.8', zh: 'Claude Opus 4.8' },
// ... etc
```
...
git commit -m "feat(claude): i18n — ~45 keys for input labels, FAQ, presets, model names"
```

---

### Task 7: UI layer — presets, token estimator, advanced collapse, beautifyClaude

**Files:**
- Modify: `src/pages/[lang]/[slug].astro` (add Claude-specific sections)

- [ ] **Step 1: Add Claude preset buttons**

In the Astro template, after the form area, add conditionally rendered Claude presets when `toolSlug === 'solopreneur-claude-api-cost-calculator'`:
  ...
  <div class="presets">
    ...
    {CLAUDE_PRESETS.map(p => (
      <button type="button" class="preset-chip"
        data-inputtokens={p.inputTokens}
        data-outputtokens={p.outputTokens}
        data-reqsperday={p.requestsPerDay}
        data-pricingmode={p.pricingMode}
        data-cachewritetokens={p.cacheWriteTokens}
        data-cachettl={p.cacheTTL}
        data-cachehitrate={p.cacheReadHitRate}
      >{t(`tools.${toolSlug}.preset.${p.slug}`)}</button>
    ))}
  </div>
)}
```

- [ ] **Step 2: Add token estimator with Claude-specific note**

```html
{isClaude && (
  <details class="token-estimator">
    <summary>{t(`tools.${toolSlug}.estimator.title`)}</summary>
    <p class="estimator-note">
      Claude uses a different tokenizer than OpenAI. For English, roughly 1.3-1.5 words per token.
      Opus 4.7+ may produce up to 35% more tokens than earlier models for the same text.
    </p>
    <textarea id="tokenEstimator" placeholder="Paste your prompt text here to estimate token count..." rows="4"></textarea>
    <div id="tokenEstimate" class="estimate-result"></div>
  </details>
)}
```

- [ ] **Step 3: Add advanced options collapse**

Wrap cache and growth inputs in a `<details>` element:

```html
{isClaude && (
  <details class="advanced-options">
    <summary>⚙️ Advanced: Prompt Caching, Growth Projections</summary>
    <!-- cacheWriteTokens, cacheTTL, cacheReadHitRate, growthRate, projectionMonths inputs -->
  </details>
)}
```

- [ ] **Step 4: Add beautifyClaude() function to client JS**

In the `<script>` block in the Astro template, add after the existing `beautifyOpenAI()`:

```javascript
function beautifyClaude(text) {
  // Apply generic bar beautifier first
  let html = beautifyBars(esc(text));
  ...
  // Wrap lines with family icons in colored spans
  html = html.replace(/^(✦|▲|◆)/gm, (match) => {
    const color = FAMILY_COLORS[match] || '#6b7280';
    return `<span style="color:${color}">${match}</span>`;
  });
  
  // Wrap 💾 Caching Breakdown section
  html = html.replace(/(💾[\s\S]*?)(?=\n\n|$)/g, 
    '<div class="section caching-breakdown">$1</div>');
  
  // Wrap 💰 Savings Insights section
  html = html.replace(/(💰[\s\S]*?)(?=\n\n|$)/g,
    '<div class="section savings-insights">$1</div>');
  
  return html;
}
```

- [ ] **Step 5: Wire isClaude flag in define:vars**

Add `isClaude: slug === 'solopreneur-claude-api-cost-calculator'` to the `define:vars` block.

- [ ] **Step 6: Commit**

```bash
git add src/pages/[lang]/[slug].astro
git commit -m "feat(claude): UI — presets, token estimator, advanced collapse, beautifyClaude()"
```

---

### Task 8: Build, verify, and finalize
...
- [ ] **Step 3: Verify customFn output matches calculate() output**

```bash
cd "D:\E\独立站\youtube-tools" && grep -n "staticExamples" src/engines/claude-api-cost-calculator.ts
```

Check that staticExamples content matches actual calculate() output format.

- [ ] **Step 4: Final commit**

```bash
git add -A
git commit -m "feat(claude): finalize v3 upgrade — build passes, typecheck clean"
```
...
1. `pnpm check` passes (typecheck + lint)
2. All 7 models appear in output with correct pricing
3. Prompt Caching math produces correct savings
4. Growth projection table totals are accurate
5. Cross-provider comparison uses cheapest Claude model
6. staticExamples match actual calculate() output exactly
7. Both English and Chinese i18n keys resolve correctly
