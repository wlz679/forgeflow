...
1. ~~**10 env-dependent Clerk/Supabase test fail hardening** — requires real `PUBLIC_CLERK_PUBLISHABLE_KEY` + `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` in `.env` or CI secrets. Affected tests: `tests/baselayout-{clerk,sync}-script.test.ts` (2), `tests/header-clerk-render.test.ts` (3), `tests/header-sync-ui.test.ts` (3), `tests/privacy-policy-sync.test.ts` (2). Not shippable in single session without external auth setup.~~ — **Closed 2026-07-19 by P26a + P28 audit**. Cascade misattribution: tests use `pk_test_xyz` placeholder via `tests/_clerk-build-helper.ts:59`. Real root cause = P2a ToolCard `listingPages` array staleness. P22 was the **origin** of this cascade misattribution (P28 spec audit `[memory/p28-p10-p14-audit-pass-shipped.md]`).
2. ~~**Extract `EXPECTED_ENGINE_COUNT` shared constant** — single source of truth referenced from all count assertions. YAGNI for now; revisit if P23+ adds engines.~~ — **Closed 2026-07-18 by P22b** (`62fc9c5`+`0f7f08d`+`ace65f1`). `tests/lib/engine-count.ts` static const + drift guard. **Lesson**: P22's "YAGNI for now" reasoning was wrong — adding the constant in same batch as the assertion fixes prevents the cascade this audit closes.
3. **`tests/run.mjs` skip-in-missing-env mode** — graceful degradation when `.env` lacks Clerk/Supabase keys. Different concern from P22 (test correctness vs runner ergonomics). **Open 2026-07-19**: partially addressed by P23b `RUN_BUILD_TESTS=1` skip-guard (build-dependent skips) + P24 CI wiring.
4. ~~**`tests/internal-links.test.ts:19` "4 entries for all 82 tools"** — also stale (should be 100), but it's testing a RELATED-COUNTS invariant, not the engine count itself. Lower priority than the 3 count asserts P22 fixes; revisit if P23 surfaces.~~ — **Closed 2026-07-19 by P25** (`402052b`). Only line 20 had stale literal; line 19 itself was non-stale per P25 pre-flight grep. Lines 22+ use dynamic `${totalTools}`. P28 audit confirmed misattribution.
```
