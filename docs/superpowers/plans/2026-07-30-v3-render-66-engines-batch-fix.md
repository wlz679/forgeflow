...
```typescript
#!/usr/bin/env node
// P138 — CI guard for v3 Business rendering coverage.
// Two checks:
//   1. Every tool slug from src/data/tools.ts must appear as a key in
//      BIZ_CONFIG_MAP (either the runtime JS in [slug].astro or one of
//      the AI-cost configs that fire earlier in calcConfig switch).
//   2. Every business tool's rendered HTML must contain the v3 section
//      card class "rounded-xl border p-4 mb-3" at least 6 times in the
//      static-results div (proves beautifySections fired, not fallback).
//
...
const V3_CARD_CLASS = 'rounded-xl border p-4 mb-3';
const MIN_V3_CARDS = 6;
...
function ensureBuilt(): void {
  ...
  console.log('[p138] dist/ missing — running pnpm build...');
...
// Read tools list from src/data/tools.ts (TS source) by extracting slug lines.
function readToolSlugs(): string[] {
  const t = readFileSync(resolve(root, 'src/data/tools.ts'), 'utf8');
  const m = t.matchAll(/slug:\s*['"]([^'"]+)['"]/g);
  const slugs: string[] = [];
  for (const x of m) slugs.push(x[1]);
  return slugs;
}

// Read runtime BIZ_CONFIG_MAP from [slug].astro script block.
function readBizConfigKeys(): Set<string> {
  const t = readFileSync(resolve(root, 'src/pages/[lang]/[slug].astro'), 'utf8');
  // Greedy match the long runtime BIZ_CONFIG_MAP line (continues on one line)
  const m = t.match(/var BIZ_CONFIG_MAP\s*=\s*\{([^}]+)\}/);
  if (!m) throw new Error('Could not find runtime BIZ_CONFIG_MAP');
  const inner = m[1];
  const keys = new Set<string>();
  for (const km of inner.matchAll(/'(solopreneur-[a-z0-9-]+)'/g)) {
    keys.add(km[1]);
  }
  return keys;
}

// AI-cost slugs use a different config pipeline (OPENAI_CONFIG, CLAUDE_CONFIG,
// etc.) — they're not in BIZ_CONFIG_MAP. Extract from the calcConfig switch.
function readAiCostSlugs(): Set<string> {
  const t = readFileSync(resolve(root, 'src/pages/[lang]/[slug].astro'), 'utf8');
  const keys = new Set<string>();
  // Match slug === 'solopreneur-...' inside isOpenAI/isClaude/isDeepSeek/isGemini
  for (const m of t.matchAll(/(?:isOpenAI|isClaude|isDeepSeek|isGemini|isImage|isTraining|isGpu|isAiApiCostComparison)\s*=\s*slug\s*===\s*['"]([^'"]+)['"]/g)) {
    keys.add(m[1]);
  }
  return keys;
}

test('P138: every tool slug is wired into BIZ_CONFIG_MAP or AI-cost switch', () => {
  ensureBuilt();
  const tools = readToolSlugs();
  const biz = readBizConfigKeys();
  const aiCost = readAiCostSlugs();
  const missing: string[] = [];
  for (const slug of tools) {
    if (!biz.has(slug) && !aiCost.has(slug)) missing.push(slug);
  }
  assert.equal(missing.length, 0, `Slugs not wired into any rendering config: ${missing.join(', ')}`);
});

test('P138: every business tool renders with >= 6 v3 section cards (en)', () => {
  ensureBuilt();
  const tools = readToolSlugs();
  const biz = readBizConfigKeys();
  const aiCost = readAiCostSlugs();
  const offenders: { slug: string; count: number }[] = [];
  for (const slug of tools) {
    if (aiCost.has(slug)) continue; // AI cost uses different visual; skip
    const htmlPath = resolve(root, 'dist/en', slug, 'index.html');
    if (!existsSync(htmlPath)) {
      offenders.push({ slug, count: -1 });
      continue;
    }
    const html = readFileSync(htmlPath, 'utf8');
    const matches = html.match(new RegExp(V3_CARD_CLASS.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || [];
    if (matches.length < MIN_V3_CARDS) offenders.push({ slug, count: matches.length });
  }
  assert.equal(offenders.length, 0, `Tools rendering < ${MIN_V3_CARDS} v3 cards: ${JSON.stringify(offenders)}`);
});

test('P138: every business tool renders with >= 6 v3 section cards (zh)', () => {
  ensureBuilt();
  const tools = readToolSlugs();
  const biz = readBizConfigKeys();
  const aiCost = readAiCostSlugs();
  const offenders: { slug: string; count: number }[] = [];
  for (const slug of tools) {
    if (aiCost.has(slug)) continue;
    const htmlPath = resolve(root, 'dist/zh', slug, 'index.html');
    if (!existsSync(htmlPath)) {
      offenders.push({ slug, count: -1 });
      continue;
    }
  ...
  assert.equal(offenders.length, 0, `Tools (zh) rendering < ${MIN_V3_CARDS} v3 cards: ${JSON.stringify(offenders)}`);
});
```
