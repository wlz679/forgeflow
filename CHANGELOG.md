...
> **最后更新:** 2026-08-19 (P140f Batch A Tier 1 Anchor Topics — Topic data layer [src/data/topics.ts with Topic interface + 15 Tier 1 anchor Topics × 2 langs = 30 new pages] + 2 new components [Breadcrumb.astro + TopicCard.astro] + categories.ts DomainId + domain field × 15 entries + i18n keys [Topic H2 names + CTA labels + breadcrumb domain segments + letter page Topics section h2] + 2 new page templates [src/pages/[lang]/[letter]/[topic]-guide.astro 6-section + [topic]-benchmark.astro 4-section data-table] + calculator page Related Topics gradient panel + 2 new build-dep guards [lang-aware Topic Guide shape + Topic Benchmark shape]; 7 atomic commits on `feature/p140f-batch-a-tier1-anchors`; M25.3 added; commit count 1094 → 1101; static pages 451 → 511 [+60]; pnpm check 1244/0/0; RUN_BUILD_TESTS=1 1266/1266/0 [50 build-dep suites, +2 new])
