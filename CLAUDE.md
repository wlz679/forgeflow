...
**Test infrastructure that catches regressions across 6 user-visible dimensions.** All 34 build-dep CI guards live in `tests/`. Run via `RUN_BUILD_TESTS=1 pnpm test:unit` (or `pnpm test:build`). Suites registered in `tests/run.mjs` skip-mode summary.
...
| **Total** | **34 build-dep suites** + 8 source-only = **42** | 6 dimensions | |
