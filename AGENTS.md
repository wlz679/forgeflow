...
> **Section boundary**: The GSTACK-CODEX MANAGED BLOCK above is the universal engineering skill index (regenerated by `npx gstack-codex init --project`). The block below is **ForgeFlowKit's project-level constitution** — project-specific constraints, architecture, and mandatory rules. The two blocks are independent; editing one does not affect the other.

# ForgeFlowKit — Project Constitution

This section provides project-level guidance to AI coding agents (Qoder / Claude Code / etc.) when working on code in this repository.
...
> **History**: pre-P46, this constitution described categories using a different taxonomy (B=Business / C=Pricing / D=HR / E=Personal / I=Investment / V=Valuation). That taxonomy was renamed/merged during P0-P16 to match `categories.ts`. P46 audit (2026-07-20) closed the drift.
...
详细机制 + 三层 scan 触发时机 + 提议协议：见 user-global memory `p140f-decision-support-system.md` 维度 3 段落。
...
Any future engine that wants the 3-band exemption must (a) cite the hard-breakpoint justification in its header comment and (b) cross-link the AGENTS.md project constitution exemption note. Both pieces are required for audit-grade clarity.
...
**Defense-in-depth invariant:** When adding a new feature that affects any of the 8 dimensions, the matching suite should catch the regression. If it doesn't, the suite is incomplete — extend it (don't bypass).

**History:** P140a opened the AdSense Compliance dimension (2026-07-31). P106-P108 completed the performance size triad (2026-07-27). P95 opened a11y dimension (2026-07-22). P86-P94 layered SEO defense (2026-07-19/20). P62-P83 closed i18n defense-in-depth (2026-07-24). P32 + P49 were the first codegen-enforced invariants (AGENTS.md engine count table, source-side guard count).
...
- **AGENTS.md project constitution is THE source of truth** for future AI sessions. Keep it accurate.
...
- **Never `git add` files under `.superpowers/`** — the `.gitignore` rule (`.superpowers/`) intentionally excludes ALL files in that path because they're runtime scratch (subagent task reports, brainstorm HTML output, etc.). P67a first surfaced this when `.superpowers/sdd/task-4-report.md` was mutated by subagents over 3 days; P70 fixed it via `git rm --cached -r .superpowers/` (7 historically-tracked scratch files removed) but the `.gitignore` rule doesn't retroactively untrack. Future subagent sessions should treat any `.superpowers/` file as scratch — write freely, but never `git add` or `git commit`. If a file under `.superpowers/` shows up in `git status`, that's a scratch mutation in progress; revert with `git checkout HEAD -- .superpowers/` rather than committing it. See P70 ship memory for the root-cause analysis.
...
这些规则针对一个反复出现的问题：AI 只完成"基础设施"或"单个入口"，但遗漏了需要同样改动的其他消费者页面。
